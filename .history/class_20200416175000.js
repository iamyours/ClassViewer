...
class ClassReader {
  constructor (arr) {
    ...
    this.byteData.data = arr;
    this.byteData.offset = 0;
  }

  readClass () {
    let c = new ClassEntry()
    c.magic = this.readU4()
    c.minor_version = this.readU2();
    c.major_version = this.readU2();
    c.constant_pool_count = this.readU2
    return c
  }

  readU1 () {
    return new U1().readBytes(this.byteData)
  }
  readU2 () {
    return new U2().readBytes(this.byteData)
  }
  readU4 () {
    return new U4().readBytes(this.byteData)
  }
}
